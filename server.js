...
function leaveRoom(ws) {
    ...
    for (const player of room.players) {
        send(player, `PLAYER_LEFT|${ws.playerId}`);
    }

    // Se a sala ficou vazia, apaga
...
wss.on("connection", (ws) => {
    ...
    console.log(`Jogador conectado: ${ws.playerId}`);

    send(ws, `CONNECTED|${ws.playerId}`);
    ...
    ws.on("message", (data) => {
        ...
        // ENTRAR|NomeDaSala|Senha
            ...
            if (!room) {
                send(ws, "ERROR|SALA_NAO_EXISTE");
                return;
            }

            if (room.password !== password) {
                send(ws, "ERROR|SENHA_INCORRETA");
                return;
            }

            if (room.players.size >= room.maxPlayers) {
                send(ws, "ERROR|SALA_CHEIA");
                return;
            }
        ...
        // MSG|qualquer informação
        else if (command === "MSG") {

            if (!ws.roomName) {
                send(ws, "ERROR|SEM_SALA");
                return;
            }
